...
static uint8_t OLED_GRAM[132][8];           //132*64
...
/**
 * @brief   set OLED cursor position        //设置OLED光标位置
 ...
 */
/**
 * @brief   摘要      在（x，y）处画一个点
 * @param   参数      x：光标的x轴
 * @param   参数      y：光标的y轴
 * @param   参数      pen: Pen_Clear, Pen_Write, Pen_Inversion @Pen_Typedef   类型
 * @retval  返回值    
 */
...
/**
 * @brief   show a number           //显示数字
 * @param   row: row of number      //行
 * @param   col: column of number   //列
 * @param   num: the number ready to show   //显示的数字
 * @param   mode: 0x01, fill number with '0'; 0x00, fill number with spaces     //数字前是否加0
 * @param   len: the length of the number   //显示的数字长度
 * @retval  None
 */
void oled_shownum(uint8_t row, uint8_t col, uint32_t num, uint8_t mode, uint8_t len)    //(行，列，数字，数字前是否加0，显示长度)
{
    uint8_t t, temp;
    uint8_t enshow = 0;
    ...
    for (t = 0; t < len; t++)
    {
        ...
        if (enshow == 0 && t < (len - 1))
        {
            ...
            {
                if (mode == 0)  
                    oled_showchar(row, col + t, ' ');   //数字前为空
                else    
                    oled_showchar(row, col + t, '0');   //数字前加0
                continue;
            }
            else
                enshow = 1;
...
/**
 * @brief   show a character string             //显示字符串
...
/**
 * @brief   formatted output in oled 128*64         //在oled 128*64中格式化输出
...
/**
 * @brief   show a chinese              //显示一个中文字符
 * @param   row: row of character
 * @param   col: column of character
 * @param   chr: the character ready to show
 * @retval  None
 */
void oled_showCHN(uint8_t row, uint8_t col, uint8_t chr)
{
    uint8_t x = col * 12;   //12倍像素
    uint8_t y = row * 12;   //12倍像素
    uint8_t temp, t, t1;
    uint8_t y0 = y;
    chr = 2 * chr ;
...
/**
 * @brief   initialize the oled module      //初始化oled模块
 * @param   None
 * @retval  None
 */
void oled_init(void)
{
    ...
    oled_write_byte(0xae, OLED_CMD);    //turn off oled panel                   关闭oled面板
    oled_write_byte(0x00, OLED_CMD);    //set low column address                设置低列地址
    oled_write_byte(0x10, OLED_CMD);    //set high column address               设置高列地址
    oled_write_byte(0x40, OLED_CMD);    //set start line address                设定起始行地址
    oled_write_byte(0x81, OLED_CMD);    //set contrast control resigter         设置对比度控制辞职
    oled_write_byte(0xcf, OLED_CMD);    //set SEG output current brightness     设置SEG输出电流亮度
    oled_write_byte(0xa1, OLED_CMD);    //set SEG/column mapping                列映射
    oled_write_byte(0xc8, OLED_CMD);    //set COM/row scan direction            设置COM/row扫描方向
    oled_write_byte(0xa6, OLED_CMD);    //set nomarl display                    设置nomarl显示
    oled_write_byte(0xa8, OLED_CMD);    //set multiplex display                 设置多路显示
    oled_write_byte(0x3f, OLED_CMD);    //1/64 duty
    oled_write_byte(0xd3, OLED_CMD);    //set display offset                    设置显示抵消
    ...
    oled_write_byte(0xaf, OLED_CMD);    //display on                             显示打开
